...
class AutoManager(ctk.CTk):
    def __init__(self):
        ...
        self.brand_img_dir = os.path.join(self.cur_dir, "img", "brands")
        self.model_img_dir = os.path.join(self.cur_dir, "img", "models")
    ...
    def ensure_folders(self):
        for d in [self.data_dir, self.prod_dir, self.img_dir, self.brand_img_dir, self.model_img_dir]:
    ...
    def setup_ui(self):
        ...
        ctk.CTkButton(self.tab_add, text="📋 Копировать все категории", command=self.copy_categories_win).pack(pady=5)
        ctk.CTkButton(self.tab_add, text="🎭 Выбрать и перенести категории", fg_color="#3498db", command=self.selective_copy_categories_win).pack(pady=5)
        ctk.CTkButton(self.tab_add, text="📦 Копировать все товары модели", command=self.copy_products_win).pack(pady=5)
        ctk.CTkButton(self.tab_add, text="🔄 Расклонировать одну запчасть", fg_color="#8e44ad", command=self.multi_copy_part_win).pack(pady=5)
    ...
    def selective_copy_categories_win(self):
        kats = self.read_js(self.files["kat"])
        models_dict = self.read_js(self.files["models"])
        
        donor_models = list(kats.keys())
        if not donor_models:
            messagebox.showwarning("Внимание", "Нет моделей с категориями в kat.js")
            return

        win = ctk.CTkToplevel(self)
        win.title("Выборочный перенос категорий")
        win.geometry("800x850")
        win.attributes("-topmost", True)

        # 1. Выбор донора
        ctk.CTkLabel(win, text="1. ОТКУДА (Модель-донор):", font=("Arial", 12, "bold")).pack(pady=5)
        src_var = ctk.StringVar(value=donor_models[0])
        src_menu = ctk.CTkOptionMenu(win, values=donor_models, variable=src_var)
        src_menu.pack(pady=5)

        # 2. Список категорий донора (Чекбоксы)
        ctk.CTkLabel(win, text="2. КАКИЕ КАТЕГОРИИ ПЕРЕНЕСТИ:", font=("Arial", 12, "bold")).pack(pady=5)
        kat_scroll = ctk.CTkScrollableFrame(win, height=200)
        kat_scroll.pack(fill="x", padx=20, pady=5)
        
        kat_checks = {}
        def refresh_kat_list(*a):
            for w in kat_scroll.winfo_children(): w.destroy()
            kat_checks.clear()
            current_kats = kats.get(src_var.get(), [])
            for k in current_kats:
                var = ctk.BooleanVar(value=True)
                cb = ctk.CTkCheckBox(kat_scroll, text=k, variable=var)
                cb.pack(anchor="w", pady=2)
                kat_checks[k] = var
        
        src_var.trace("w", refresh_kat_list)
        refresh_kat_list()

        # 3. Список моделей целей
        ctk.CTkLabel(win, text="3. КУДА (Выберите модели):", font=("Arial", 12, "bold")).pack(pady=5)
        target_scroll = ctk.CTkScrollableFrame(win, height=350)
        target_scroll.pack(fill="both", expand=True, padx=20, pady=5)

        target_checks = {}
        for bid in models_dict:
            for m in models_dict[bid]:
                var = ctk.BooleanVar()
                m_name = m['name']
                cb = ctk.CTkCheckBox(target_scroll, text=f"[{bid.upper()}] {m_name}", variable=var)
                cb.pack(anchor="w", pady=2)
                target_checks[m_name] = var

        def do_selective_copy():
            selected_kats = [k for k, v in kat_checks.items() if v.get()]
            selected_targets = [m for m, v in target_checks.items() if v.get()]

            if not selected_kats:
                messagebox.showwarning("Ошибка", "Не выбрано ни одной категории")
                return
            if not selected_targets:
                messagebox.showwarning("Ошибка", "Не выбрано ни одной модели-цели")
                return

            for target in selected_targets:
                if target not in kats:
                    kats[target] = ["Все"]
                
                # Добавляем только те, которых еще нет
                for new_k in selected_kats:
                    if new_k not in kats[target]:
                        kats[target].append(new_k)

            self.write_js(self.files["kat"], "CATEGORIES_DATA", kats)
            messagebox.showinfo("Успех", f"Категории перенесены в {len(selected_targets)} моделей")
            win.destroy()

        ctk.CTkButton(win, text="ВЫПОЛНИТЬ ПЕРЕНОС", fg_color="#3498db", height=50, command=do_selective_copy).pack(pady=20)
    ...
    def multi_copy_part_win(self):
        brands = self.read_js(self.files["brands"])
        models_dict = self.read_js(self.files["models"])
        if not brands: return
        win = ctk.CTkToplevel(self); win.title("Расклонировать запчасть"); win.geometry("700x900"); win.attributes("-topmost", True)
        
        ctk.CTkLabel(win, text="1. ВЫБЕРИТЕ ЗАПЧАСТЬ (ИСТОЧНИК)", font=("Arial", 12, "bold")).pack(pady=5)
        src_b_var = ctk.StringVar(value=brands[0]['name'])
        ctk.CTkOptionMenu(win, values=[b['name'] for b in brands], variable=src_b_var).pack(pady=2)
        src_m_var = ctk.StringVar()
        src_m_menu = ctk.CTkOptionMenu(win, variable=src_m_var); src_m_menu.pack(pady=2)
        src_p_var = ctk.StringVar()
        src_p_menu = ctk.CTkOptionMenu(win, variable=src_p_var); src_p_menu.pack(pady=2)

        def up_src_m(*a):
            bid = next(b['id'] for b in brands if b['name'] == src_b_var.get())
            ms = [m['name'] for m in models_dict.get(bid, [])]
            src_m_menu.configure(values=ms)
            if ms: src_m_var.set(ms[0])
        def up_src_p(*a):
            bid = next(b['id'] for b in brands if b['name'] == src_b_var.get())
            mid = src_m_var.get().lower().replace(" ", "_")
            data = self.read_js(os.path.join(self.prod_dir, f"{bid}_{mid}.js"))
            ps = [f"{p['art']} | {p['brand']}" for p in data]
            src_p_menu.configure(values=ps)
            if ps: src_p_var.set(ps[0])
        src_b_var.trace("w", up_src_m); src_m_var.trace("w", up_src_p); up_src_m()

        ctk.CTkLabel(win, text="2. ВЫБЕРИТЕ МОДЕЛИ (КУДА КОПИРОВАТЬ)", font=("Arial", 12, "bold")).pack(pady=(15, 5))
        scroll = ctk.CTkScrollableFrame(win, width=650, height=400); scroll.pack(pady=5, padx=10, fill="both", expand=True)
        target_checks = {}
        for bid in models_dict:
            for m in models_dict[bid]:
                var = ctk.BooleanVar()
                key = f"{bid} | {m['name']}"
                ctk.CTkCheckBox(scroll, text=key, variable=var).pack(anchor="w", pady=1)
                target_checks[key] = var

        def do_multi_copy():
            if not src_p_var.get(): return
            src_bid = next(b['id'] for b in brands if b['name'] == src_b_var.get())
            src_mid = src_m_var.get().lower().replace(" ", "_")
            src_data = self.read_js(os.path.join(self.prod_dir, f"{src_bid}_{src_mid}.js"))
            part_art = src_p_var.get().split(" | ")[0]
            part_to_copy = next((p for p in src_data if p['art'] == part_art), None)
            
            if not part_to_copy: return
            count = 0
            for key, var in target_checks.items():
                if var.get():
                    t_bid, t_mname = key.split(" | ")
                    t_mid = t_mname.lower().replace(" ", "_")
                    t_path = os.path.join(self.prod_dir, f"{t_bid}_{t_mid}.js")
                    t_data = self.read_js(t_path)
                    if not any(p['art'] == part_to_copy['art'] for p in t_data):
                        t_data.append(part_to_copy)
                        self.write_js(t_path, f"PRODUCTS_{t_bid.upper()}_{t_mid.upper()}", t_data)
                        count += 1
            messagebox.showinfo("ОК", f"Запчасть скопирована в {count} моделей"); win.destroy()

        ctk.CTkButton(win, text="ЗАПУСТИТЬ КОПИРОВАНИЕ", fg_color="#8e44ad", height=50, command=do_multi_copy).pack(pady=10)
    ...
    def add_brand_win(self):
        win = ctk.CTkToplevel(self); win.title("Добавить марку"); win.geometry("400x300"); win.attributes("-topmost", True)
        en = ctk.CTkEntry(win, placeholder_text="Название марки (например: BMW)"); en.pack(pady=20, padx=20, fill="x")
        
        lbl = ctk.CTkLabel(win, text="Фото: не выбрано")
        lbl.pack()
        ...
        def pick_img():
            ...
            if f:
                self.temp_img_path = f
                lbl.configure(text=f"Фото: {os.path.basename(f)}")

        ctk.CTkButton(win, text="🖼️ Выбрать логотип", command=pick_img).pack(pady=10)
        ...
        def save():
            ...
            if self.temp_img_path:
                img_name = f"{bid}{os.path.splitext(self.temp_img_path)[1]}"
                shutil.copy(self.temp_img_path, os.path.join(self.brand_img_dir, img_name))

            data.append({"id": bid, "name": name.upper(), "img": img_name})
            self.write_js(self.files["brands"], "BRANDS_DATA", data)
            win.destroy(); messagebox.showinfo("ОК", "Марка добавлена")
    ...
    def add_model_win(self):
        brands = self.read_js(self.files["brands"])
        if not brands: return
        win = ctk.CTkToplevel(self); win.geometry("400x400"); win.attributes("-topmost", True); win.title("Добавить модель")
        
        b_names = [b['name'] for b in brands]; b_var = ctk.StringVar(value=b_names[0])
        ctk.CTkLabel(win, text="Выберите марку:").pack(pady=(10,0))
        ctk.CTkOptionMenu(win, values=b_names, variable=b_var).pack(pady=10)
        
        en = ctk.CTkEntry(win, placeholder_text="Название модели (например: X5)"); en.pack(pady=10, padx=20, fill="x")
        ...
        ctk.CTkButton(win, text="🖼️ Выбрать фото модели", command=pick_img).pack(pady=10)
        ...
        def save():
            ...
            img_name = "default.jpg"
            if self.temp_img_path:
                clean_m = re.sub(r'[^a-zA-Z0-9]', '_', mname.lower())
                img_name = f"{bid}_{clean_m}{os.path.splitext(self.temp_img_path)[1]}"
                shutil.copy(self.temp_img_path, os.path.join(self.model_img_dir, img_name))

            if bid not in models: models[bid] = []
            models[bid].append({"name": mname, "img": img_name})
            self.write_js(self.files["models"], "MODELS_DATA", models)
            win.destroy(); messagebox.showinfo("ОК", "Модель добавлена")
