...
function renderPartsUI() {
    ...
    // ИЗМЕНЕНИЕ: Формируем список кнопок категорий строго по kat.js
    let cats = [];
    if (typeof CATEGORIES_DATA !== 'undefined' && CATEGORIES_DATA[currentModel]) {
        // Добавляем "Все" только один раз в начало
        cats = ["Все", ...CATEGORIES_DATA[currentModel]];
    } else {
        cats = ["Все"];
    }
...
function renderPartsList() {
    // Получаем разрешенные категории из kat.js
    const allowedCategories = (typeof CATEGORIES_DATA !== 'undefined' && CATEGORIES_DATA[currentModel]) 
        ? CATEGORIES_DATA[currentModel].map(c => c.trim()) 
        : [];

    // 1. Сначала фильтруем по "зависимости" (только то, что есть в kat.js)
    const validParts = allModelParts.filter(p => {
        if (!p.type) return false;
        return allowedCategories.includes(p.type.trim());
    });

    // 2. Затем фильтруем по выбранной кнопке
    const filtered = (currentCategory === 'Все') 
        ? validParts 
        : validParts.filter(p => p.type && p.type.trim() === currentCategory.trim());
...
function filterByCategory(cat) { 
    currentCategory = cat; 
    currentPage = 1; 
    renderPartsUI(); // Перерисовываем UI, чтобы обновить активный класс тега
}
...
function initSmartSearch() {
    ...
    if (typeof BRANDS_DATA !== 'undefined') {
        BRANDS_DATA.forEach(b => searchIndex.push({ name: b.name, type: 'Марка', id: b.id }));
    }
    
    if (typeof MODELS_DATA !== 'undefined') {
        Object.keys(MODELS_DATA).forEach(bid => {
            MODELS_DATA[bid].forEach(m => {
                searchIndex.push({ name: m.name, type: 'Модель', brandId: bid, modelName: m.name });
                
                const mid = m.name.toLowerCase().replace(/\s+/g, '_');
                const script = document.createElement('script');
                script.src = `data/products/${bid.toLowerCase()}_${mid}.js`;
                script.onload = () => {
                    const varName = `PRODUCTS_${bid.toUpperCase()}_${m.name.toUpperCase().replace(/\s+/g, '_')}`;
                    if (window[varName]) {
                        window[varName].forEach(p => {
                            searchIndex.push({ 
                                name: `${p.brand} ${p.art} ${p.type}`, 
                                desc: (p.desc || "").toLowerCase(),
                                art: (p.art || "").toLowerCase(),
                                type: 'Запчасть', 
                                brandId: bid, 
                                modelName: m.name,
                                fullData: p 
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        const matches = searchIndex.filter(i => {
            ...
            const inDesc = i.desc ? i.desc.includes(q) : false;
            const inArt = i.art ? i.art.includes(q) : false;
            return inName || inDesc || inArt;
        }).slice(0, 15);

        if (matches.length > 0) {
            suggestions.innerHTML = matches.map(i => {
                let displayTitle = i.name;
                let icon = "📂";
                if(i.type === 'Запчасть') icon = "📦";
                if(i.type === 'Марка') icon = "🚗";

                return `<div class="suggestion-item" onclick="handleSearchClick('${i.type}', '${i.id || ''}', '${i.brandId || ''}', '${i.modelName || ''}', ${i.fullData ? `'${encodeURIComponent(JSON.stringify(i.fullData))}'` : 'null'})">
                    <span style="color: var(--accent); font-size: 0.8em; margin-right:5px;">${icon}</span>
...
function handleSearchClick(type, id, bid, mname, partJson) {
    ...
    if (type === 'Марка') {
        ...
        currentBrand = bid; 
        loadPartsCatalog(mname);
    } else if (type === 'Запчасть' && partJson) {
        const p = JSON.parse(decodeURIComponent(partJson));
        const imgs = (p.images && p.images.length > 0) ? p.images : ['default.jpg'];
        const productData = encodeURIComponent(JSON.stringify({
            images: imgs,
            desc: p.desc || "Описание отсутствует",
